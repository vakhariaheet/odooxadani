...
4. **Review Style Guide for This Module:**
   - **Reminder Button:** Ghost variant with mail icon
   - **Disabled State:** Show "Reminder Sent" when recently sent
   - **Icon Size:** 16px (h-4 w-4) for button icons
   - **Button Placement:** In invoice action column or card footer
   - See `guidelines/STYLE_GUIDE.md` → "M07: Email Reminders" section
